...
ex> 멤버와 사물함의 관계를 예로 들면, 멤버는 주테이블, 사물함은 대상테이블
...
일대일 관계 중 대상 테이블에 외래 키가 있는 단방향 관계를 JPA에서는 지원하지 않는다. 그리고 이런 모양으로 매핑할 수 있는 방법도 없다.
...
객체는 테이블과 다르게 객체 2개로 @ManyToMany를 사용하면 다대다 관계를 만들 수 있다. 

ex> 회원과 상품의 관계로 설명해본다.
...
- 복합 키를 사용하지 않고 간단히 다대다 관계를 구성하는 방법



매핑 클래스에 DB에서 발급하는 식별자를 PK로 하고, member와 product의 key는 FK로 사용하는 방법
...
* 비식별관계 : 받아온 식별자는 외래 키로만 사용하고 새로운 식별자를 추가 (ORM이 간단해지므로 추천)
...
ORM의 상속을 DB로 표현하는 방법

출처 : 자바 ORM 표준 JPA 프로그래밍 p244

ex> A를 상속한 B, C가 있는 경우

1. 각각의 테이블로 변환 (A, B, C 테이블을 세 개 만듬)
   - 장점 
     - 테이블이 정규화 된다.
...
3. 서브타입 테이블로 변환 (BA. BC 테이블을 두 개 만듬)
   - 장점
